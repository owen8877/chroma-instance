...
def to_cor():
    return r"""
\def\BlankColor{rgb:white,0.01}
\def\BlueColor{rgb:blue,5;white,1;green,2}
\def\RedColor{rgb:red,5;white,1}
\def\GreyColor{rgb:white,5;black,5}
...
# Conv
def to_Conv(name, y_label=256, z_label=256, n_filer=(64, 64), offset="(0,0,0)", to="(0,0,0)", width=(1, 1), height=40,
            depth=40, caption=" ", fill_color=r"\ConvColor"):
    return r"""
...
# Conv,Conv,relu
# Bottleneck
def to_ConvConvRelu(name, y_label=256, z_label=256, n_filer=(64, 64), offset="(0,0,0)", to="(0,0,0)", width=(2, 2),
                    height=40,
                    depth=40, caption=" "):
    return r"""
\pic[shift={ """ + offset + """ }] at """ + to + """ 
...
# SoftMax
def to_SoftMax(name, s_filer=" ", n_filer=" ", offset="(0,0,0)", to="(0,0,0)", width=1.5, height=3, depth=25,
               opacity=0.8, caption=" "):
    return r"""
\pic[shift={""" + offset + """}] at """ + to + """ 
    {Box={
...
def to_inj(name, offset="(0,0,0)", to="(0,0,0)", radius=2.5, opacity=0.6):
    return r"""
\pic[shift={""" + offset + """}] at """ + to + """ 
    {Ball={
        name=""" + name + """,
...
def to_connection_thin(of, to, of_dir='east', to_dir='west'):
    if of_dir: of = '(' + of + '-' + of_dir + ')'
    if to_dir: to = '(' + to + '-' + to_dir + ')'

    return r"""
\draw [connection, line width=1pt, opacity=.5]  """ + of + """    -- node {\midarrowe} """ + to + """;
"""


def to_upsample(of, to):
    return fr"""\draw[densely dashed, opacity=0.5]
...
def to_FullyConnected(name, s_filer=" ", n_filer=" ", offset="(0,0,0)", to="(0,0,0)", width=1.5, height=3, depth=25,
                      opacity=0.8, caption=" ", zlabelposition='midway'):
    return r"""
\pic[shift={""" + offset + """}] at """ + to + """ 
    {Box={
        name=""" + name + """,

...
from chroma_gan import dataClass as data
import numpy as np
import cv2
...
def sample_images(pretrained, test_dir, batch_size, out_dir):
    # avg_cost = 0
    # avg_cost2 = 0
    # avg_cost3 = 0
    # avg_ssim = 0
    ...
    test_data = data.DATA(test_dir)
    assert batch_size <= test_data.size, "The batch size should be smaller or equal to the number of testing images --> modify it in config.py"
    total_batch = int(test_data.size / batch_size)+1
    ...
    for b in tqdm(range(total_batch)):
        ...
        for i in range(batch_size):
            ...
            save_path = os.path.join(out_dir, filelist[i] + "psnr_reconstructed.jpg")
